...
$checkmail = mysqli_query($link,"SELECT * FROM users WHERE username='".make_safe($_GET['username'])."' AND password='".md5(make_safe($_GET['password']))."'");

$getInfo = mysqli_fetch_array($checkmail);
$getNumAccount = mysqli_num_rows($checkmail);

if($getNumAccount > 0 ){
	$update_user_ip = mysqli_query($link,"UPDATE users SET ip='".make_safe($real_ip_adress)."' WHERE id='".make_safe($getInfo['id'])."'");
	if($update_user_ip){
if($getInfo["active"] == 1){
